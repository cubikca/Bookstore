    ...
    public class OrganizationRepository : RepositoryBase<Organization, Models.Organization>, IOrganizationRepository
    {
        ...
        public override async Task<Organization> Save(Organization model)
        {
            try
            {
                ...
                var entity = await db.Organizations.SingleOrDefaultAsync(o => o.Id == organization.Id);
                ...
                var result = Mapper.Map<Organization>(await db.Organizations.SingleAsync(o => o.Id == entity.Id)); 
                scope.Complete();
                return result;
            }
            catch (Exception ex)
        ...
        public override async Task<bool> Remove(Guid id)
        {
            try
            {
                using var scope = new TransactionScope(TransactionScopeOption.Required,
                    TransactionScopeAsyncFlowOption.Enabled);
                await using var db = DbFactory.CreateDbContext();
                var entity = await db.Organizations.SingleOrDefaultAsync(o => o.Id == id && !o.Deleted);
        ...
        public override async Task<ICollection<Organization>> FindAll()
        {
            ...
            {
                await using var db = DbFactory.CreateDbContext();
                var entities = await db.Organizations 
                    .Where(o => !o.Deleted)
                    .ToListAsync();
